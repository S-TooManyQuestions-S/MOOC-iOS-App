...
namespace Udemy
{
    public class UdemyCourse : Course
    {
        public UdemyCourse(string title, double relevancy_score, string image_240x135, string url, int id)
            : base(title, new Rating(relevancy_score / 1000 > 5 ? 5 : relevancy_score / 1000), new Image(image_240x135), new CurrentInfo("Udemy", string.Format(APIpath, id), detailsPath + url))
        { }
